	...
	table(table, options, callback) {
		return new Promise((resolve, reject) => {
			try {
				...
				const calcTopTextToAlignVertically = (text, width, cellPadding, align, valign, rowDistance, columnSpacing, rectCell) => {
				...
				const onFirePageAdded = () => {
					// startX = this.page.margins.left;
					console.log('On fire page added!!!!')
				...
				const separationsColumn = (type, x, y, height, width, opacity, color) => {
					type || (type = 'vertical'); // top | header | horizontal | vertical 
					// disabled
				...
				const separationsRow = (type, x, y, width, opacity, color) => {
					...
					type || (type = 'horizontal'); // top | header | horizontal | vertical 
				...
				const addHeader = () => { 
					...
					if(!options.hideHeader && table.headers.length > 0) {
						...
						table.headers.forEach( (dataHeader, i) => {
							...
							// align vertically
							let topTextToAlignVertically = calcTopTextToAlignVertically(label, width, cellPadding, align, valign, rowDistance, columnSpacing, rectCell);
					...
					if(!options.hideHeader) {
						...
						separationsRow('top', startX, startY - columnSpacing - (rowDistance * 2))
						// Separation line between headers and rows
						separationsRow('header', startX, rowBottomY);
						originalStartY = startY - columnSpacing - (rowDistance * 2);
					} else {
						rowBottomY = startY;
					}
				...
				table.datas.forEach((row, i) => {
					...
					table.headers.forEach(( dataHeader, index) => {
						...
						// align vertically
						let topTextToAlignVertically = calcTopTextToAlignVertically(text, cellPadding, width, align, valign, rowDistance, columnSpacing, rectCell);
				...
				table.rows.forEach((row, i) => {
					...
					row.forEach((cell, index) => {
						...
						// align vertically
						let topTextToAlignVertically = calcTopTextToAlignVertically(cell, cellPadding, columnSizes[index], align, valign, rowDistance, columnSpacing, rectCell);
				...
				table.headers.forEach((dataHeader) => {
					separationsColumn('vertical', dataHeader.startX, originalStartY, this.y - originalStartY);
				})
				separationsColumn('vertical', tableWidth, originalStartY, this.y - originalStartY);
				this.moveDown(); // break
